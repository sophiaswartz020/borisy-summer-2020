// Title: 1.1-sizeparam-ROIsave.ijm
...
function processFile(input, output, file) {
	...
	for (j=1; j<9; j++) { 
		...
		run("Analyze Particles...", "size=0.10-Infinity display include summarize add"); // incorporated an "add" command to track ROIs

		//Save and close ROI manager; edit suggested by Jennifer Gundrum
        if (roiManager("count")>0) {
        	roiManager("Save", output + "RoiSet_segCh_" + j + ".zip");
        	roiManager("Reset");
        }
