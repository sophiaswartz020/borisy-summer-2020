//Title: 1.6-batchprocessing-inputoutput.ijm
...
// Sets an imageID for the duplicate image stack
...
} // keep track of the i in the different for-loops
...
function processFile(input, output, file) {
	...
	function SaveAndClose(file, output, j) {
		saveAs("Results", output + titleOrig + "/Channel " + j + "/summary-dup-Ch" + j + ".csv");
		close("summary-dup-Ch" + i + ".csv");
		saveAs("Results", output + titleOrig + "/Channel " + j + "/results-dup-Ch" + j + ".csv");  
		close("Results");
		saveAs("Tiff", output + titleOrig + "/Channel " + j + "/segimage-dup-Ch" + j + ".tif");
